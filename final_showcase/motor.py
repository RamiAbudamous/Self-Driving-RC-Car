...
def brake(car):
    car.motor_ch.pulse_width(0)
    # car.motor_ch.pulse_width(convert_speed(0))

# def convert_speed(input_speed):
#     speed = max(0, min(config.MAX_SPEED, input_speed)) # clip speed
#     return int(speed*(19200/10000)) #scale speed and return

def go(car):
    car.motor_ch.pulse_width_percent(int(speed_detect(config.MAX_SPEED)))
    return
    ...
    car.motor_ch.pulse_width_percent(int(speed_detect(speed)))
    # car.motor_ch.pulse_width(convert_speed(speed_detect(speed)))
    print(f"angle is {car.angle_turn}, speed is {speed}")
...
def offroad(car):
    ...
    if config.KILL==1:
        ...
        # speed detector test code
        # car.motor_ch.pulse_width(convert_speed(speed_detect(max(config.MAX_SPEED-car.brake_counter, config.MIN_SPEED)))) # slow down a lot
        # if car.brake_counter>50:
        #     car.brake_counter=0

        car.motor_ch.pulse_width_percent(speed_detect(config.MIN_SPEED)) # slow down a lot
...
def speed_detect(speed):
    ...
    # print(f"speed is {speed}, max is {config.MAX_SPEED}, min is {config.MIN_SPEED}")
    ...
    # fin_speed = int(speed_ratio * speed)
    # # print(f"fin speed is {fin_speed}")
    # return fin_speed
